...
async function ensureSqliteColumns(instance: Db) {
  ...
  await addColumnIfMissing("users", "auto_enroll", "ALTER TABLE users ADD COLUMN auto_enroll INTEGER NOT NULL DEFAULT 1");
  await addColumnIfMissing("users", "auto_confirm_plan", "ALTER TABLE users ADD COLUMN auto_confirm_plan INTEGER NOT NULL DEFAULT 0");
